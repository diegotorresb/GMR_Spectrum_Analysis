...
def read_data_from_folder(master_folder):
    ...
    """
    MRAD_PER_TURN = 6.63  # Base conversion factor
    CALIBRATION_FACTOR = 437/439  # Calibration factor from membrane tilt
    MRAD_PER_TURN_CALIBRATED = MRAD_PER_TURN * CALIBRATION_FACTOR
...
def plot_contour(data_dict, title_suffix, ax):
    ...
    # Plot contour
    im = ax.pcolormesh(wavelengths, angles, transmittance_data,
                       shading='auto',
                       vmin=0.0,
                       vmax=1.2,
                       cmap='inferno')
    ...
    # Create invisible lines initially
    vline = ax.axvline(wavelengths[0], color='k', linestyle='--', alpha=0.5, visible=False)
    hline = ax.axhline(angles[0], color='k', linestyle='--', alpha=0.5, visible=False)
